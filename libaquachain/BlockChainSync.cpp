...
std::ostream& dev::aqua::operator<<(std::ostream& _out, SyncStatus const& _sync)
{
	_out << "protocol: " << _sync.protocolVersion << endl;
	_out << "state: " << AquachainHost::stateName(_sync.state) << " ";
...
BlockChainSync::BlockChainSync(AquachainHost& _host):
	m_host(_host),
...
void BlockChainSync::abortSync()
{
	RecursiveGuard l(x_sync);
	resetSync();
	host().foreachPeer([&](std::shared_ptr<AquachainPeer> _p)
	{
		_p->abortSync();
		return true;
	});
...
void BlockChainSync::onPeerStatus(std::shared_ptr<AquachainPeer> _peer)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
	...
	else if (_peer->m_protocolVersion != host().protocolVersion() && _peer->m_protocolVersion != AquachainHost::c_oldProtocolVersion)
...
bool BlockChainSync::requestDaoForkBlockHeader(std::shared_ptr<AquachainPeer> _peer)
{
	// DAO challenge
...
void BlockChainSync::syncPeer(std::shared_ptr<AquachainPeer> _peer, bool _force)
{
...
{
	host().foreachPeer([this](std::shared_ptr<AquachainPeer> _p)
	{
		syncPeer(_p, false);
		return true;
	});
...
void BlockChainSync::requestBlocks(std::shared_ptr<AquachainPeer> _peer)
{
...
void BlockChainSync::clearPeerDownload(std::shared_ptr<AquachainPeer> _peer)
{
...
void BlockChainSync::onPeerBlockHeaders(std::shared_ptr<AquachainPeer> _peer, RLP const& _r)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
...
void BlockChainSync::onPeerBlockBodies(std::shared_ptr<AquachainPeer> _peer, RLP const& _r)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
	size_t itemCount = _r.itemCount();
...
void BlockChainSync::onPeerNewBlock(std::shared_ptr<AquachainPeer> _peer, RLP const& _r)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
...
void BlockChainSync::onPeerNewHashes(std::shared_ptr<AquachainPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes)
{
	RecursiveGuard l(x_sync);
	DEV_INVARIANT_CHECK;
