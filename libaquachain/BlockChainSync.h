...
{
public:
	BlockChainSync(AquachainHost& _host);
	~BlockChainSync();
	...
	void onPeerStatus(std::shared_ptr<AquachainPeer> _peer);

	/// Called by peer once it has new block headers during sync
	void onPeerBlockHeaders(std::shared_ptr<AquachainPeer> _peer, RLP const& _r);

	/// Called by peer once it has new block bodies
	void onPeerBlockBodies(std::shared_ptr<AquachainPeer> _peer, RLP const& _r);

	/// Called by peer once it has new block bodies
	void onPeerNewBlock(std::shared_ptr<AquachainPeer> _peer, RLP const& _r);

	void onPeerNewHashes(std::shared_ptr<AquachainPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes);
...
private:
	...
	AquachainHost& host() { return m_host; }
	AquachainHost const& host() const { return m_host; }

	void resetSync();
	void syncPeer(std::shared_ptr<AquachainPeer> _peer, bool _force);
	void requestBlocks(std::shared_ptr<AquachainPeer> _peer);
	void clearPeerDownload(std::shared_ptr<AquachainPeer> _peer);
	void clearPeerDownload();
	void collectBlocks();
	bool requestDaoForkBlockHeader(std::shared_ptr<AquachainPeer> _peer);
...
private:
	...
	AquachainHost& m_host;
	Handler<> m_bqRoomAvailable;				///< Triggered once block queue has space for more blocks
	mutable RecursiveMutex x_sync;
	std::set<std::weak_ptr<AquachainPeer>, std::owner_less<std::weak_ptr<AquachainPeer>>> m_daoChallengedPeers; ///> Peers to which we've sent DAO challenge request
	...
	std::map<std::weak_ptr<AquachainPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<AquachainPeer>>> m_headerSyncPeers; ///< Peers to m_downloadingSubchain number map
	std::map<std::weak_ptr<AquachainPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<AquachainPeer>>> m_bodySyncPeers; ///< Peers to m_downloadingSubchain number map
