...
#include <libaquachain/BlockChainSync.h>
#include "CommonNet.h"
...
struct AquachainHostTrace: public LogChannel { static const char* name(); static const int verbosity = 6; };

/**
 * @brief The AquachainHost class
 * @warning None of this is thread-safe. You have been warned.
 * @doWork Syncs to peers and sends new blocks and transactions.
 */
class AquachainHost: public p2p::HostCapability<AquachainPeer>, Worker
{
public:
	/// Start server, but don't listen.
	AquachainHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId);

	/// Will block on network process events.
	virtual ~AquachainHost();
	...
	void foreachPeer(std::function<bool(std::shared_ptr<AquachainPeer>)> const& _f) const;
...
private:
	...
	std::tuple<std::vector<std::shared_ptr<AquachainPeer>>, std::vector<std::shared_ptr<AquachainPeer>>, std::vector<std::shared_ptr<p2p::SessionFace>>> randomSelection(unsigned _percent = 25, std::function<bool(AquachainPeer*)> const& _allow = [](AquachainPeer const*){ return true; });
	...
	OverlayDB const& m_db;					///< References to DB, needed for some of the Aquachain Protocol responses.
	...
	std::shared_ptr<AquachainHostDataFace> m_hostData;
	std::shared_ptr<AquachainPeerObserverFace> m_peerObserver;
};
