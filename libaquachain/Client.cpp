...
std::ostream& dev::aqua::operator<<(std::ostream& _out, ActivityReport const& _r)
{
...
void Client::init(p2p::Host* _extNet, fs::path const& _dbPath, fs::path const& _snapshotDownloadPath, WithExisting _forceAction, u256 _networkId)
{
    ...
    // create Aquachain capability only if we're not downloading the snapshot
    if (_snapshotDownloadPath.empty())
    {
        auto host = _extNet->registerCapability(
            make_shared<AquachainHost>(bc(), m_stateDB, m_tq, m_bq, _networkId));
        m_host = host;

        _extNet->addCapability(host, AquachainHost::staticName(),
            AquachainHost::c_oldProtocolVersion);  // TODO: remove this once v61+ protocol is common
    }
...
{
    // TODO: more precise check on whaquaer the txs match.
