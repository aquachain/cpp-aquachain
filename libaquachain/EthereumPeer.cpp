...
AquachainPeer::AquachainPeer(std::shared_ptr<SessionFace> _s, HostCapabilityFace* _h, unsigned _i, CapDesc const& _cap):
    Capability(_s, _h, _i),
...
void AquachainPeer::init(unsigned _hostProtocolVersion, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash, shared_ptr<AquachainHostDataFace> _hostData, shared_ptr<AquachainPeerObserverFace> _observer)
{
...
bool AquachainPeer::isRude() const
{
    auto s = session();
    if (s)
...
unsigned AquachainPeer::askOverride() const
{
    std::string static const badGaqua = "Gaqua/v0.9.27";
    auto s = session();
    if (!s)
        return c_maxBlocksAsk;
    if (s->info().clientVersion.substr(0, badGaqua.size()) == badGaqua)
        return 1;
    bytes const& d = s->repMan().data(*s, name());
    return d.empty() ? c_maxBlocksAsk : RLP(d).toInt<unsigned>(RLP::LaissezFaire);
}

void AquachainPeer::setRude()
{
    auto s = session();
    if (!s)
        return;
...
void AquachainPeer::setIdle()
{
    setAsking(Asking::Nothing);
}

void AquachainPeer::requestStatus(u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash)
{
    ...
    prep(s, StatusPacket, 5)
                    << (latest ? m_hostProtocolVersion : AquachainHost::c_oldProtocolVersion)
                    << _hostNetworkId
...
void AquachainPeer::requestBlockHeaders(unsigned _startNumber, unsigned _count, unsigned _skip, bool _reverse)
{
    if (m_asking != Asking::Nothing)
    {
...
void AquachainPeer::requestBlockHeaders(h256 const& _startHash, unsigned _count, unsigned _skip, bool _reverse)
{
    if (m_asking != Asking::Nothing)
    {
        clog(NetWarn) << "Asking headers while requesting " << ::toString(m_asking);
...
void AquachainPeer::requestBlockBodies(h256s const& _blocks)
{
    requestByHashes(_blocks, Asking::BlockBodies, GetBlockBodiesPacket);
}

void AquachainPeer::requestNodeData(h256s const& _hashes)
{
    requestByHashes(_hashes, Asking::NodeData, GetNodeDataPacket);
}

void AquachainPeer::requestReceipts(h256s const& _blocks)
{
    requestByHashes(_blocks, Asking::Receipts, GetReceiptsPacket);
}

void AquachainPeer::requestByHashes(h256s const& _hashes, Asking _asking, SubprotocolPacketType _packetType)
{
    if (m_asking != Asking::Nothing)
    {
...
void AquachainPeer::setAsking(Asking _a)
{
    m_asking = _a;
...
bool AquachainPeer::interpret(unsigned _id, RLP const& _r)
{
    ...
    case StatusPacket:
    {
        ...
        setIdle();
        observer->onPeerStatus(dynamic_pointer_cast<AquachainPeer>(dynamic_pointer_cast<AquachainPeer>(shared_from_this())));
        break;
    }
    case TransactionsPacket:
    {
        observer->onPeerTransactions(dynamic_pointer_cast<AquachainPeer>(dynamic_pointer_cast<AquachainPeer>(shared_from_this())), _r);
        break;
    }
    ...
    case BlockHeadersPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerBlockHeaders(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), _r);
        }
        break;
    }
    ...
    case BlockBodiesPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerBlockBodies(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), _r);
        }
        break;
    }
    case NewBlockPacket:
    {
        observer->onPeerNewBlock(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), _r);
        break;
    }
    case NewBlockHashesPacket:
    {
        ...
        observer->onPeerNewHashes(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), hashes);
        break;
    }
    ...
    case NodeDataPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerNodeData(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), _r);
        }
        break;
    }
    case ReceiptsPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerReceipts(dynamic_pointer_cast<AquachainPeer>(shared_from_this()), _r);
        }
        break;
    }
    default:
