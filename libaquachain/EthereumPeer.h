...
class AquachainPeerObserverFace
{
public:
	virtual ~AquachainPeerObserverFace() {}

	virtual void onPeerStatus(std::shared_ptr<AquachainPeer> _peer) = 0;

	virtual void onPeerTransactions(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerBlockHeaders(std::shared_ptr<AquachainPeer> _peer, RLP const& _headers) = 0;

	virtual void onPeerBlockBodies(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerNewHashes(std::shared_ptr<AquachainPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes) = 0;

	virtual void onPeerNewBlock(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerNodeData(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerReceipts(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) = 0;

	virtual void onPeerAborting() = 0;
};

class AquachainHostDataFace
{
public:
	virtual ~AquachainHostDataFace() {}
...
/**
 * @brief The AquachainPeer class
 * @todo Document fully.
 * @todo make state transitions thread-safe.
 */
class AquachainPeer: public p2p::Capability
{
	friend class AquachainHost; //TODO: remove this
	friend class BlockChainSync; //TODO: remove this

public:
	/// Basic constructor.
	AquachainPeer(std::shared_ptr<p2p::SessionFace> _s, p2p::HostCapabilityFace* _h, unsigned _i, p2p::CapDesc const& _cap);

	/// Basic destructor.
	virtual ~AquachainPeer();
	...
	void init(unsigned _hostProtocolVersion, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash, std::shared_ptr<AquachainHostDataFace> _hostData, std::shared_ptr<AquachainPeerObserverFace> _observer);
...
private:
	...
	std::weak_ptr<AquachainPeerObserverFace> m_observer;
	std::weak_ptr<AquachainHostDataFace> m_hostData;
};
