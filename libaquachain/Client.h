...
/**
 * @brief Main API hub for interfacing with Aquachain.
 */
...
public:
    ...
    /// Get the block.
    dev::aqua::Block block(h256 const& _blockHash, PopulationStatistics* o_stats) const;

    /// Get the object representing the current state of Aquachain.
    dev::aqua::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
    ...
    std::unique_ptr<StateImporterFace> createStateImporter() { return dev::aqua::createStateImporter(m_stateDB); }
    std::unique_ptr<BlockChainImporterFace> createBlockChainImporter() { return dev::aqua::createBlockChainImporter(m_bc); }
...
protected:
    ...
    std::weak_ptr<AquachainHost> m_host;     ///< Our Aquachain Host. Don't do anything if we can't lock.
