...
class DummyLastBlockHashes: public aqua::LastBlockHashesFace
{
public:
...
bool Block::sync(BlockChain const& _bc, h256 const& _block, BlockHeader const& _bi)
{
    ...
    else
    {
        ...
        if (m_state.db().lookup(bi.stateRoot()).empty())	// TODO: API in State for this?
        {
            ...
            cwarn << "Try rescuing the database by running: aqua --rescue";
...
//						cnote << "TX took:" << t.elapsed() * 1000;
                ...
                {
                    // Somaquaing else went wrong - drop it.
...
/*		if (fromPending(i).transactionsFrom(m_transactions[i].from()) != m_transactions[i].nonce())
        {
            cwarn << "GAAA Somaquaing went wrong! " << fromPending(i).transactionsFrom(m_transactions[i].from()) << "!=" << m_transactions[i].nonce();
        }*/
//#endif
    }
