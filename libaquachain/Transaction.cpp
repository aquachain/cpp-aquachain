...
std::ostream& dev::aqua::operator<<(std::ostream& _out, ExecutionResult const& _er)
{
	_out << "{" << _er.gasUsed << ", " << _er.newAddress << ", " << toHex(_er.output) << "}";
	return _out;
}

TransactionException dev::aqua::toTransactionException(Exception const& _e)
{
...
std::ostream& dev::aqua::operator<<(std::ostream& _out, TransactionException const& _er)
{
	switch (_er)
	{
