...
*/
/** @file AquachainHost.cpp
 * @author Gav Wood <i@gavwood.com>
 * @date 2014
 */

#include "AquachainHost.h"
...
#include <libaquacore/Exceptions.h>
#include "BlockChain.h"
#include "TransactionQueue.h"
#include "BlockQueue.h"
#include "AquachainPeer.h"
...
unsigned const AquachainHost::c_oldProtocolVersion = 62; //TODO: remove this once v63+ is common
static unsigned const c_maxSendTransactions = 256;

char const* const AquachainHost::s_stateNames[static_cast<int>(SyncState::Size)] = {"NotSynced", "Idle", "Waiting", "Blocks", "State"};

#if defined(_WIN32)
const char* AquachainHostTrace::name() { return EthPurple "^" EthGray "  "; }
#else
const char* AquachainHostTrace::name() { return EthPurple "⧫" EthGray " "; }
#endif
...
namespace
{
class AquachainPeerObserver: public AquachainPeerObserverFace
{
public:
	AquachainPeerObserver(shared_ptr<BlockChainSync> _sync, TransactionQueue& _tq): m_sync(_sync), m_tq(_tq) {}

	void onPeerStatus(std::shared_ptr<AquachainPeer> _peer) override
	{
		try
		{
		...
		{
			// "fix" for https://github.com/aquachain/webthree-umbrella/issues/300
			clog(NetWarn) << "Failed invariant during sync, restarting sync";
			m_sync->restartSync();
		}
	}

	void onPeerTransactions(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(AquachainHostTrace) << "Transactions (" << dec << itemCount << "entries)";
	...
	void onPeerBlockHeaders(std::shared_ptr<AquachainPeer> _peer, RLP const& _headers) override
	{
		try
		{
	...
	void onPeerBlockBodies(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) override
	{
		try
		{
	...
	void onPeerNewHashes(std::shared_ptr<AquachainPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes) override
	{
		try
		{
	...
	void onPeerNewBlock(std::shared_ptr<AquachainPeer> _peer, RLP const& _r) override
	{
		try
		{
	...
	void onPeerNodeData(std::shared_ptr<AquachainPeer> /* _peer */, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(AquachainHostTrace) << "Node Data (" << dec << itemCount << "entries)";
	}

	void onPeerReceipts(std::shared_ptr<AquachainPeer> /* _peer */, RLP const& _r) override
	{
		unsigned itemCount = _r.itemCount();
		clog(AquachainHostTrace) << "Receipts (" << dec << itemCount << "entries)";
	}
...
class AquachainHostData: public AquachainHostDataFace
{
public:
	AquachainHostData(BlockChain const& _chain, OverlayDB const& _db): m_chain(_chain), m_db(_db) {}
...
AquachainHost::AquachainHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId):
	HostCapability<AquachainPeer>(),
	Worker		("aquasync"),
	m_chain		(_ch),
	m_db(_db),
	m_tq		(_tq),
	...
	m_hostData(make_shared<AquachainHostData>(m_chain, m_db))
{
	// TODO: Composition would be better. Left like that to avoid initialization
	//       issues as BlockChainSync accesses other AquachainHost members.
	m_sync.reset(new BlockChainSync(*this));
	m_peerObserver = make_shared<AquachainPeerObserver>(m_sync, m_tq);
	m_latestBlockSent = _ch.currentHash();
	m_tq.onImport([this](ImportResult _ir, h256 const& _h, h512 const& _nodeId) { onTransactionImported(_ir, _h, _nodeId); });
}

AquachainHost::~AquachainHost()
{
	terminate();
}

bool AquachainHost::ensureInitialised()
{
	if (!m_latestBlockSent)
	{
		// First time - just initialise.
		m_latestBlockSent = m_chain.currentHash();
		clog(AquachainHostTrace) << "Initialising: latest=" << m_latestBlockSent;
...
void AquachainHost::completeSync()
{
	m_sync->completeSync();
}

void AquachainHost::doWork()
{
	...
	{
		m_lastTick = now;
		foreachPeer([](std::shared_ptr<AquachainPeer> _p) { _p->tick(); return true; });
	}
...
void AquachainHost::maintainTransactions()
{
	// Send any new transactions.
	unordered_map<std::shared_ptr<AquachainPeer>, std::vector<size_t>> peerTransactions;
	auto ts = m_tq.topTransactions(c_maxSendTransactions);
	{
		...
		for (size_t i = 0; i < ts.size(); ++i)
		{
			...
			auto peers = get<1>(randomSelection(0, [&](AquachainPeer* p) { return p->m_requireTransactions || (unsent && !p->m_knownTransactions.count(t.sha3())); }));
	...
	foreachPeer([&](shared_ptr<AquachainPeer> _p)
	{
		bytes b;
		unsigned n = 0;
		...
		if (n || _p->m_requireTransactions)
		{
			...
			_p->sealAndSend(ts);
			clog(AquachainHostTrace) << "Sent" << n << "transactions to " << _p->session()->info().clientVersion;
		}
...
void AquachainHost::foreachPeer(std::function<bool(std::shared_ptr<AquachainPeer>)> const& _f) const
{
	...
	for (auto s: sessions)
		if (!_f(capabilityFromSession<AquachainPeer>(*s.first)))
			return;

	sessions = peerSessions(c_oldProtocolVersion); //TODO: remove once v61+ is common
	std::sort(sessions.begin(), sessions.end(), sessionLess);
	for (auto s: sessions)
		if (!_f(capabilityFromSession<AquachainPeer>(*s.first, c_oldProtocolVersion)))
			return;
}

tuple<vector<shared_ptr<AquachainPeer>>, vector<shared_ptr<AquachainPeer>>, vector<shared_ptr<SessionFace>>> AquachainHost::randomSelection(unsigned _percent, std::function<bool(AquachainPeer*)> const& _allow)
{
	vector<shared_ptr<AquachainPeer>> chosen;
	vector<shared_ptr<AquachainPeer>> allowed;
	vector<shared_ptr<SessionFace>> sessions;

	size_t peerCount = 0;
	foreachPeer([&](std::shared_ptr<AquachainPeer> _p)
	{
		if (_allow(_p.get()))
		{
...
void AquachainHost::maintainBlocks(h256 const& _currentHash)
{
	// Send any new blocks.
	...
	if (detailsFrom.totalDifficulty < detailsTo.totalDifficulty)
	{
		if (diff(detailsFrom.number, detailsTo.number) < 20)
		{
			...
			clog(AquachainHostTrace) << "Sending a new block (current is" << _currentHash << ", was" << m_latestBlockSent << ")";

			h256s blocks = get<0>(m_chain.treeRoute(m_latestBlockSent, _currentHash, false, false, true));

			auto s = randomSelection(25, [&](AquachainPeer* p){
				DEV_GUARDED(p->x_knownBlocks)
					return !p->m_knownBlocks.count(_currentHash);
				return false;
			});
			for (shared_ptr<AquachainPeer> const& p: get<0>(s))
			...
			for (shared_ptr<AquachainPeer> const& p: get<1>(s))
			{
				RLPStream ts;
...
bool AquachainHost::isSyncing() const
{
	return m_sync->isSyncing();
}

SyncStatus AquachainHost::status() const
{
	return m_sync->status();
}

void AquachainHost::onTransactionImported(ImportResult _ir, h256 const& _h, h512 const& _nodeId)
{
	auto session = host()->peerSession(_nodeId);
	if (!session)
		return;

	std::shared_ptr<AquachainPeer> peer = capabilityFromSession<AquachainPeer>(*session);
	if (!peer)
		peer = capabilityFromSession<AquachainPeer>(*session, c_oldProtocolVersion);
	if (!peer)
		return;
...
shared_ptr<Capability> AquachainHost::newPeerCapability(shared_ptr<SessionFace> const& _s, unsigned _idOffset, p2p::CapDesc const& _cap)
{
	auto ret = HostCapability<AquachainPeer>::newPeerCapability(_s, _idOffset, _cap);

	auto cap = capabilityFromSession<AquachainPeer>(*_s, _cap.second);
	assert(cap);
	cap->init(
		protocolVersion(),
		m_networkId,
