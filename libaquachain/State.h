...
#include <libaquachain/CodeSizeCache.h>
...
/**
 * Model of an Aquachain state, essentially a facade for the trie.
 *
 ...
 * The changelog is managed by savepoint(), rollback() and commit() maquaods.
 */
class State
{
	friend class ExtVM;
...
public:
	...
	/// which uses it. If you have no preexisting database then set BaseState to somaquaing other
	...
	/// Populate the state from the given AccountMap. Just uses dev::aqua::commit().
	void populateFrom(AccountMap const& _map);

	/// @returns the set containing all addresses currently in use in Aquachain.
	...
	/// @param _commitBehaviour whaquaer or not to remove empty accounts during commit.
