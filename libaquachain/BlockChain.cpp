...
std::ostream& dev::aqua::operator<<(std::ostream& _out, BlockChain const& _bc)
{
...
db::Slice dev::aqua::toSlice(h256 const& _h, unsigned _sub)
{
...
db::Slice dev::aqua::toSlice(uint64_t _n, unsigned _sub)
{
#if ALL_COMPILERS_ARE_CPP11_COMPLIANT
    ...
    if (!m_blocksDB || !m_extrasDB)
    {
        ...
        else
        {
            cwarn <<
                ...
                "already open. You appear to have another instance of aquachain running. Bailing.";
...
//  _bq.tick(*this);
    ...
    for (VerifiedBlock const& block: blocks)
    {
        do {
            ...
            catch (dev::aqua::AlreadyHaveBlock const&)
            {
                cwarn << "ODD: Import queue contains already imported block";
                continue;
            }
            catch (dev::aqua::UnknownParent)
            {
...
void BlockChain::insert(VerifiedBlockRef _block, bytesConstRef _receipts, bool _mustBeNew)
{
    ...
    // togaquaer with an "ensureCachedWithUpdatableLock(l)" maquaod.
...
{
    //@tidy This is a behemoth of a maquaod - could do to be split into a few smaller ones.
...
vector<unsigned> BlockChain::withBlockBloom(LogBloom const& _b, unsigned _earliest, unsigned _latest, unsigned _level, unsigned _index) const
{
    ...
    for (unsigned o = obegin; o < oend; ++o)
        ...
        {
            // This level has somaquaing like what we want.
            if (_level > 0)
...
Block BlockChain::genesisBlock(OverlayDB const& _db) const
{
    ...
    {
        ret.noteChain(*this);
        dev::aqua::commit(m_params.genesisState, ret.mutableState().m_state);        // bit horrible. maybe consider a better way of constructing it?
