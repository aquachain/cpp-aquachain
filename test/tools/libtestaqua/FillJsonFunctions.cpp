...
json_spirit::mObject fillJsonWithStateChange(
    State const& _stateOrig, aqua::State const& _statePost, aqua::ChangeLog const& _changeLog)
{
    ...
    aqua::ChangeLog changeLog = _changeLog;
    std::sort(changeLog.begin(), changeLog.end(),
        [](const aqua::Change& lhs, const aqua::Change& rhs) { return lhs.address < rhs.address; });
    ...
    while (i < changeLog.size())
    {
        ...
        while (i < changeLog.size() && changeLog[i].address == currentAddress)
        {
            ...
            aqua::Change change = changeLog.at(i);
            switch (change.kind)
            {
...
json_spirit::mObject fillJsonWithState(State const& _state, aqua::AccountMaskMap const& _map)
{
