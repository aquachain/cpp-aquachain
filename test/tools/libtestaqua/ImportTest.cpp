...
ImportTest::ImportTest(json_spirit::mObject const& _input, json_spirit::mObject& _output):
	m_statePre(0, OverlayDB(), aqua::BaseState::Empty),
	m_statePost(0, OverlayDB(), aqua::BaseState::Empty),
...
void ImportTest::makeBlockchainTestFromStateTest(set<aqua::Network> const& _networks) const
{
    ...
    for (auto const& tr : m_transactions)
    {
        ...
        State s = State(0, OverlayDB(), aqua::BaseState::Empty);
        AccountMaskMap m;
...
set<aqua::Network> ImportTest::getAllNetworksFromExpectSections(
...
void ImportTest::checkBalance(aqua::State const& _pre, aqua::State const& _post, bigint _miningReward)
{
	bigint preBalance = 0;
	...
	//account could destroy aquaer if it suicides to itself
...
std::tuple<aqua::State, ImportTest::ExecOutput, aqua::ChangeLog> ImportTest::executeTransaction(aqua::Network const _sealEngineNetwork, aqua::EnvInfo const& _env, aqua::State const& _preState, aqua::Transaction const& _tr)
{
	assert(m_envInfo);

	State initialState = _preState;
	ExecOutput out(std::make_pair(aqua::ExecutionResult(), aqua::TransactionReceipt(h256(), u256(), aqua::LogEntries())));
	try
	{
		...
		aqua::ChangeLog changeLog = initialState.changeLog();
...
void ImportTest::importTransaction (json_spirit::mObject const& _o, aqua::Transaction& o_tr)
{
        ...
        if (network.count(netIdToString(tr.netId)) || network.count("ALL"))
                ...
                (inArray(v, tr.valInd) || v[0] == -1))
            {
                ...
                if (_expects.count("result"))
                {
                    ...
                    aqua::AccountMaskMap stateMap;
                    State expectState(0, OverlayDB(), aqua::BaseState::Empty);
...
void ImportTest::traceStateDiff()
{
	...
	for(auto const& tr : m_transactions)
	{
		...
		if ((d == tr.dataInd || d == -1) && (g == tr.gasInd || g == -1) && (v == tr.valInd || v == -1))
		{
			...
			dev::LogOutputStream<aqua::StateTrace, false>() << log.str();
