...
#include <test/tools/libtestaqua/TestUtils.h>
...
{
	m_json = loadJsonFromFile(toTestFilePath(getCommandLineArgument("--aqua_testfile")));
}

void ParallelFixture::enumerateThreads(std::function<void()> callback) const
{
	size_t threadsCount = std::stoul(getCommandLineArgument("--aqua_threads"), nullptr, 10);
...
void BlockChainFixture::enumerateBlockchains(std::function<void(Json::Value const&, dev::aqua::BlockChain const&, State state)> callback) const
{
...
void ClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::aqua::ClientBase&)> callback) const
{
...
void ParallelClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::aqua::ClientBase&)> callback) const
{
	ClientBaseFixture::enumerateClients([this, &callback](Json::Value const& _json, dev::aqua::ClientBase& _client) -> void
	{
