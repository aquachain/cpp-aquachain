...
{
	void enumerateBlockchains(std::function<void(Json::Value const&, dev::aqua::BlockChain const&, dev::aqua::State state)> callback) const;
};

struct ClientBaseFixture: public BlockChainFixture
{
	void enumerateClients(std::function<void(Json::Value const&, dev::aqua::ClientBase&)> callback) const;
};
