...
public:
	TestClient(Secret const& _authority):
		...
		m_bc(aqua::ChainParams(c_genesisInfoTestBasicAuthority), m_path)
	{
...
h256s subs(bytesConstRef _node)
{
		...
		// branch
	{
		...
		if (r[1].size() == 32)	// TODO: check whaquaer it's really an extension node, or whaquaer it's just an terminal-node with 32 bytes of data.
