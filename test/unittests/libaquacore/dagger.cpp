...
BOOST_AUTO_TEST_CASE(basic_test)
{
	...
	js::mValue v;
	string const s = contentsString(testPath / fs::path("aquahash_tests.json"));
	BOOST_REQUIRE_MESSAGE(s.length() > 0, "Contents of 'aquahash_tests.json' is empty. Have you cloned the 'tests' repo branch develop?");
	js::read_string(s, v);
	for (auto& i: v.get_obj())
	{
		...
		aqua::Nonce nonce(o["nonce"].get_str());
