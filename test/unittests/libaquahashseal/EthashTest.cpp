...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumWithoutUncles)
{
	...
	Ethash aquahash;
	aquahash.setChainParams(params);
	...
	BOOST_CHECK_EQUAL(aquahash.calculateDifficulty(header, parentHeader), 999024);
}
...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumWithUncles)
{
	...
	BOOST_CHECK_EQUAL(aquahash.calculateDifficulty(header, parentHeader), 999512);
}
...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumMaxAdjustment)
{
	...
	BOOST_CHECK_EQUAL(aquahash.calculateDifficulty(header, parentHeader), 951688);
}
