...
void EthashCPUMiner::workLoop()
{
	...
	aquahash_return_value aquahashReturn;
	...
	{
		aquahashReturn = aquahash_full_compute(dag->full, *(aquahash_h256_t*)w.headerHash().data(), tryNonce);
		h256 value = h256((uint8_t*)&aquahashReturn.result, h256::ConstructFromPointer);
		if (value <= boundary && submitProof(EthashProofOfWork::Solution{(h64)(u64)tryNonce, h256((uint8_t*)&aquahashReturn.mix_hash, h256::ConstructFromPointer)}))
			break;
		if (!(hashCount % 100))
