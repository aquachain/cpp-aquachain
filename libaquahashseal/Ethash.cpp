...
#include <libaquachain/Interface.h>
...
void Ethash::verify(Strictness _s, BlockHeader const& _bi, BlockHeader const& _parent, bytesConstRef _block) const
{
	...
	if (_s == CheckEverything && _bi.parentHash() && !verifySeal(_bi))
	{
		...
		ex << errinfo_aquahashResult(make_tuple(er.value, er.mixHash));
...
bool Ethash::quickVerifySeal(BlockHeader const& _bi) const
{
	...
	auto n = nonce(_bi);
	auto b = boundary(_bi);
	bool ret = !!aquahash_quick_check_difficulty(
		(aquahash_h256_t const*)h.data(),
		(uint64_t)(u64)n,
		(aquahash_h256_t const*)m.data(),
		(aquahash_h256_t const*)b.data());
	return ret;
}
...
bool Ethash::verifySeal(BlockHeader const& _bi) const
{
	...
	if (!pre && slow)
	{
		cwarn << "WARNING: evaluated result gives true whereas aquahash_quick_check_difficulty gives false.";
