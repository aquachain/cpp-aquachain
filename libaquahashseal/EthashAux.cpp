...
EthashAux* dev::aqua::EthashAux::s_this = nullptr;
...
{
	return aquahash_get_cachesize((uint64_t)_header.number());
}

uint64_t EthashAux::dataSize(uint64_t _blockNumber)
{
	return aquahash_get_datasize(_blockNumber);
}
...
EthashAux::LightAllocation::LightAllocation(h256 const& _seedHash)
{
	...
	light = aquahash_light_new(blockNumber);
	if (!light)
		BOOST_THROW_EXCEPTION(ExternalFunctionFailure("aquahash_light_new()"));
	size = aquahash_get_cachesize(blockNumber);
}
...
EthashAux::FullAllocation::FullAllocation(aquahash_light_t _light, aquahash_callback_t _cb)
{
//	cdebug << "About to call aquahash_full_new...";
	full = aquahash_full_new(_light, _cb);
//	cdebug << "Called OK.";
	if (!full)
	{
		clog(DAGChannel) << "DAG Generation Failure. Reason: "  << strerror(errno);
		BOOST_THROW_EXCEPTION(ExternalFunctionFailure("aquahash_full_new"));
	}
}
...
{
	return bytesConstRef((byte const*)aquahash_full_dag(full), size());
}
...
EthashAux::FullType EthashAux::full(h256 const& _seedHash, bool _createIfMissing, function<int(unsigned)> const& _f)
{
	...
	{
		s_dagCallback = _f;
//		cnote << "Loading from libaquahash...";
...
{
	aquahash_return_value_t r = aquahash_full_compute(full, *(aquahash_h256_t*)_headerHash.data(), (uint64_t)(u64)_nonce);
	if (!r.success)
...
{
	aquahash_return_value r = aquahash_light_compute(light, *(aquahash_h256_t*)_headerHash.data(), (uint64_t)(u64)_nonce);
	if (!r.success)
		BOOST_THROW_EXCEPTION(DAGCreationFailure());
	return EthashProofOfWork::Result{h256((uint8_t*)&r.result, h256::ConstructFromPointer), h256((uint8_t*)&r.mix_hash, h256::ConstructFromPointer)};
}
