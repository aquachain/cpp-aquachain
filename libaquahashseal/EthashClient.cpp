...
EthashClient& dev::aqua::asEthashClient(Interface& _c)
{
...
Ethash* EthashClient::aquahash() const
{
...
{
    return aquahash()->farm().isMining();
}

WorkingProgress EthashClient::miningProgress() const
{
    if (isMining())
        return aquahash()->farm().miningProgress();
...
std::tuple<h256, h256, h256> EthashClient::getEthashWork()
{
    ...
    aquahash()->manuallySetWork(m_sealingInfo);
...
{
    aquahash()->manuallySubmitWork(_mixHash, _nonce);
    return true;
}
