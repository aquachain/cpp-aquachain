...
#include <libaquahashseal/EthashAux.h>
#include <libevm/VM.h>
#include <libevm/VMFactory.h>
#include <libaquacore/KeyManager.h>
#include <libaquachain/Defaults.h>
#include <libaquachain/SnapshotImporter.h>
#include <libaquachain/SnapshotStorage.h>
...
string aquaCredits(bool _interactive = false)
{
...
void version()
{
    cout << "aqua version " << dev::Version << "\n";
    cout << "aqua network protocol version: " << dev::aqua::c_protocolVersion << "\n";
    cout << "Client database version: " << dev::aqua::c_databaseVersion << "\n";
...
void stopSealingAfterXBlocks(aqua::Client* _c, unsigned _start, unsigned& io_mining)
{
    try
    {
...
//  double blockFees = 15.0;
    ...
    addNetworkingOption("bootstrap,b",
        "Connect to the default Aquachain peer servers (default unless --no-discovery used).");
    addNetworkingOption("no-bootstrap",
        "Do not connect to the default Aquachain peer servers (default only when --no-discovery is "
        "used).");
    ...
    if (vm.count("mainnet"))
    {
        chainParams = ChainParams(genesisInfo(aqua::Network::MainNetwork), genesisStateRoot(aqua::Network::MainNetwork));
        chainConfigIsSet = true;
    }
    if (vm.count("ropsten"))
    {
        chainParams = ChainParams(genesisInfo(aqua::Network::Ropsten), genesisStateRoot(aqua::Network::Ropsten));
        chainConfigIsSet = true;
    }
    if (vm.count("ask"))
    {
    ...
    if (!configJSON.empty())
    {
        ...
        catch (...)
        {
            ...
            cerr << "sample: \n" << genesisInfo(aqua::Network::MainNetworkTest) << "\n";
            return 0;
        }
    }
    ...
    if (g_logVerbosity > 0)
        cout << EthGrayBold "cpp-aquachain, a C++ Aquachain client" EthReset << "\n";
    ...
    dev::WebThreeDirect web3(WebThreeDirect::composeClientVersion("aqua"), getDataDir(),
        snapshotPath, chainParams, withExisting, nodeMode == NodeMode::Full ? caps : set<string>(),
        netPrefs, &nodesState, testingMode);

    if (!extraData.empty())
        web3.aquachain()->setExtraData(extraData);

    auto toNumber = [&](string const& s) -> unsigned {
        if (s == "latest")
            return web3.aquachain()->number();
        if (s.size() == 64 || (s.size() == 66 && s.substr(0, 2) == "0x"))
            return web3.aquachain()->blockChain().number(h256(s));
        try
        {
            return stol(s);
        }
    ...
    if (mode == OperationMode::Export)
    {
        ...
        {
            bytes block = web3.aquachain()->blockChain().block(web3.aquachain()->blockChain().numberHash(i));
            switch (exportFormat)
            {
    ...
    if (mode == OperationMode::Import)
    {
        ...
        while (in.peek() != -1)
        {
            ...
            switch (web3.aquachain()->queueBlock(block, safeImport))
            {
            ...
            tuple<ImportRoute, bool, unsigned> r = web3.aquachain()->syncQueue(10);
            imported += get<2>(r);
            ...
            if ((unsigned)e >= last + 10)
            {
                ...
                auto d = e - last;
                cout << i << " more imported at " << (round(i * 10 / d) / 10) << " blocks/s. " << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.aquachain()->number() << ")" << "\n";
                last = (unsigned)e;
        ...
        while (moreToImport)
        {
            ...
            tie(ignore, moreToImport, ignore) = web3.aquachain()->syncQueue(100000);
        }
        double e = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - t).count() / 1000.0;
        cout << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.aquachain()->number() << ")\n";
        return 0;
    }
            ...
            if (!keyManager.load(masterPassword) && masterSet)
            {
                while (true)
                {
                    ...
                    if (keyManager.load(masterPassword))
                        break;
                    cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " << (getDataDir("aquachain") / fs::path("keys.info")).string() << "\n";
                }
            }
        }
        else
    ...
    if (mode == OperationMode::ImportSnapshot)
    {
        try
        {
            auto stateImporter = web3.aquachain()->createStateImporter();
            auto blockChainImporter = web3.aquachain()->createBlockChainImporter();
            SnapshotImporter importer(*stateImporter, *blockChainImporter);

            auto snapshotStorage(createSnapshotStorage(filename));
            importer.import(*snapshotStorage, web3.aquachain()->blockChain().genesisHash());
...
//  std::shared_ptr<aqua::BasicGasPricer> gasPricer = make_shared<aqua::BasicGasPricer>(u256(double(aquaer / 1000) / aquaerPrice), u256(blockFees * 1000));
    std::shared_ptr<aqua::TrivialGasPricer> gasPricer = make_shared<aqua::TrivialGasPricer>(askPrice, bidPrice);
    aqua::Client* c = nodeMode == NodeMode::Full ? web3.aquachain() : nullptr;
    if (c)
    {
    ...
    if (testingMode)
        ...
        authenticator = [&](TransactionSkeleton const& _t, bool isProxy) -> bool {
            ...
            string r = getResponse(_t.userReadable(isProxy,
                ...
                {
                    h256 contractCodeHash = web3.aquachain()->postState().codeHash(_t.to);
    ...
    if (ipc)
    {
        ...
        accountHolder.reset(new SimpleAccountHolder([&](){ return web3.aquachain(); }, getAccountPassword, keyManager, authenticator));
        auto aquaFace = new rpc::Eth(*web3.aquachain(), *accountHolder.get());
        rpc::TestFace* testEth = nullptr;
        if (testingMode)
            testEth = new rpc::Test(*web3.aquachain());

        jsonrpcIpcServer.reset(new FullServer(
            aquaFace, new rpc::Net(web3),
            new rpc::Web3(web3.clientVersion()), new rpc::Personal(keyManager, *accountHolder, *web3.aquachain()),
            new rpc::AdminEth(*web3.aquachain(), *gasPricer.get(), keyManager, *sessionManager.get()),
            new rpc::AdminNet(web3, *sessionManager.get()),
            new rpc::Debug(*web3.aquachain()),
            testEth
        ));
        auto ipcConnector = new IpcServer("gaqua");
