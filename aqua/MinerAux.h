...
//
// Bob has already done just that in https://github.com/bobsummerwill/cpp-aquachain/commits/cmake_fixes/aquaminer,
// and we should apply those changes back to 'develop'.  It is probably best to defer that cleanup
// until after attempting to backport the Genoil aquaminer changes, because they are fairly extensive
...
//
// See https://github.com/bobsummerwill/cpp-aquachain/commit/53af845268b91bc6aa1dab53a6eac675157a072b
// See https://github.com/bobsummerwill/cpp-aquachain/commit/3b9e581d7c04c637ebda18d3d86b5a24d29226f4
//
// More generally, the fact that nearly all of the code for aquaminer is actually in the 'MinerAux.h'
...
inline std::string credits()
{
	...
	out
		<< "cpp-aquachain " << dev::Version << endl
		<< "  By cpp-aquachain contributors, (c) 2013-2018." << endl
	...
	bool interpretOption(size_t& i, vector<string> const& argv)
	{
		...
		else if ((arg == "-w" || arg == "--check-pow") && i + 4 < argc)
		{
			...
			try
			{
				...
				cout << "  and " << r.value << " = aquahash(" << powHash << ", " << h64(m) << ")" << endl;
