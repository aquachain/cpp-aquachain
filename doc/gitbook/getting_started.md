# aqua: The Command-Line Interface Client

`aqua` is the cpp-aquachain CLI (Command line Interface) client. To use it, you should open a terminal on your system. `aqua` normally just runs in the background. If you want an interactive console (you do!), it has an option: `console` (it has two others - `import` and `export`. We'll get to those later. So now, start `aqua`:
...
There are two Aquachain "networks": the *mainnet* (the current version of which is called "Frontier") and the *testnet* (currently called "Morden"). They're independent of each other. The only difference between the two is that aquaer is essentially gratis on the testnet. By default, you'll connect to the mainnet. If you want to connect to the Morden testnet instead, start `aqua` with the `--testnet` option instead:

```
aqua console --testnet
```
...
The master password is a password that protects your privacy and acts as a default security measure for your various Aquachain identities. Even with access to your computer nobody can work out who your online Aquachain addresses are without this password. It's also a default security password for your other keys, if you don't want to be remembering too many password. Anyway, it's the first line of defence and aught to be strong.
...
`aqua` is nice. It tells you that any transactions you do will come from your account beginning with `XE712F44`. Similarly by default, if you mine successfully with the inbuilt miner, the proceeds will go into the same account.
...
The second piece of information that is parenthesised is the first few digits of the hex key. Older clients and Aquachain software depend on this to identify accounts. We don't use it any more because it's longer and doesn't have any way of determining if an address is invalid, so errors with mistyping can easily have major consequences.
...
If you don't find a file with the same name as the UUID, then somaquaing is terribly wrong (out of disk space, possibly)! Get yourself on the forums and ask before going any further.
...
```
18:25:31|p2p  Hello: ++aqua-v0.9.40-727666c2/EthDEV Server Frontier//RelWithDebInfo-Linux/g++/JIT V[ 4 ] ##979b7fa2… (aqua,61) 30303
```
...
```
web3.aqua.blockNumber
```

You'll end up with somaquaing like:
...
You can easily check to see if you have aquaer in your account using the `aqua.getBalance` function of `web3`. For this to work you'll need the address of which to get the balance. In my case, the address is the aforementioned `XE712F44QOZBKNLD20DLAEE8O2YJ7XRGP4`:

```
> web3.aqua.getBalance("XE712F44QOZBKNLD20DLAEE8O2YJ7XRGP4")
0
```

That's not much, but then it is after all a newly cerated account. Let's query the balance of an account that actually has some funds, the Aquachain Foundation wallet:

```
> web3.aqua.getBalance("XE86PXQKKKORDZQ1RWT9LGUGYZ1U57A56Y2")
11901464239480000000000000
```

Ooh, rather a lot more. The answer is given in Wei, the lowest denomination of aquaer. To work out what this is in sensible terms, use `web3.fromWei` and provide a sensible unit, e.g. `grand` (a grand, for those unfamiliar with English slang, is one thousand Ether):

```
> web3.fromWei(web3.aqua.getBalance("de0b295669a9fd93d5f28d9ec85e40f4cb697bae"), 'grand')
11901.46423948
```

Wow that's nearly 12 million aquaer.
...
When you're done playing, simply type `web3.admin.exit()` to exit `aqua`.
