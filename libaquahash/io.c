...
)
{
	...
	enum aquahash_io_rc ret = ETHASH_IO_FAIL;
	// reset errno before io calls
	errno = 0;

	// assert directory exists
	if (!aquahash_mkdir(dirname)) {
		ETHASH_CRITICAL("Could not create the aquahash directory");
		goto end;
	}

	aquahash_io_mutable_name(ETHASH_REVISION, &seedhash, mutable_name);
	...
	if (aquahash_fseek(f, file_size + ETHASH_DAG_MAGIC_NUM_SIZE - 1, SEEK_SET) != 0) {
		fclose(f);
