...
// TODO: This one is specifically for Aquachain, but we can make it generic in due course.
// TODO: hidden-partition style key-store.
/**
 * @brief High-level manager of password-encrypted keys for Aquachain.
 * Usage:
 *
 * Call exists() to check whaquaer there is already a database. If so, get the master password from
...
public:
	...
	static boost::filesystem::path defaultPath() { return getDataDir("aquachain") / boost::filesystem::path("keys.info"); }
