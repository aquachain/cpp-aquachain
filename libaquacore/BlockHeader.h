 ...
 * from some given RLP block serialisation with the static fromHeader(), through the maquaod
 ...
 * constructor BlockHeader(bytesConstRef) and manually with the populate() maquaod. These will
 ...
 * populateFromParent() maquaod. The genesis block info may be retrieved with genesis() and the
 * corresponding RLP block created with createGenesisBlock().
 *
 * To determine the header hash without the nonce (for sealing), the maquaod hash(WithoutNonce) is
 * provided.
 *
